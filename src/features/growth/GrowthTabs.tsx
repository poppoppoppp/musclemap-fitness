  ...
  return (
    <div role="tablist" aria-label="成长分类" className="grid grid-cols-3 rounded-full border border-lime-300/20 bg-black/30 p-1">
      {tabs.map((tab) => {
        ...
        return (
          <button key={tab.id} type="button" role="tab" aria-selected={selected} onClick={() => onChange(tab.id)} className={`min-h-12 rounded-full px-4 text-sm font-black transition duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-lime-200 ${selected ? 'bg-lime-300 text-[#10130d] shadow-[0_8px_24px_rgba(163,230,53,0.16)]' : 'text-zinc-400 hover:text-zinc-100'}`}>
            {tab.label}
          </button>
        );
      })}
