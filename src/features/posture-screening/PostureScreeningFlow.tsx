...
import type { PostureScreeningDraftStep, PostureScreeningRepository } from '../../repositories/postureScreeningRepository';
...
type FlowStep = 'boundary' | 'safety' | 'concern' | 'movement' | 'review';
...
export default function PostureScreeningFlow({ repository }: { repository: PostureScreeningRepository }) {
  ...
  const persist = (nextStep: FlowStep, input = buildInput()) => {
    const saved = repository.saveDraft({ currentStep: nextStep, answers: input, photoMeasurements: [] });
    if (!saved.ok) {
  ...
  const complete = (input: PostureScreeningInput) => {
  ...
  return (
    <>
      ...
      {step === 'review' ? <ReviewStep onBack={() => persist('movement')} onComplete={() => complete(buildInput())} /> : null}
    </>
  );
}
...
function ReviewStep({ onBack, onComplete }: { onBack: () => void; onComplete: () => void }) {
  ...
  return (
    <section className="mt-7" aria-labelledby="review-title">
      ...
      <button type="button" onClick={onComplete} className="mt-5 min-h-12 w-full rounded-xl bg-lime-300 px-4 text-sm font-black text-[#10130d] outline-none focus-visible:ring-2 focus-visible:ring-lime-100">暂不使用照片，生成结果</button>
...
function normalizeStep(step?: PostureScreeningDraftStep): FlowStep {
  if (step === 'safety' || step === 'concern' || step === 'movement' || step === 'review') return step;
  if (step === 'follow-up') return 'concern';
  if (step === 'photo') return 'review';
  return 'boundary';
}
