...
export default function ExercisePickerSheet({ open, existingExerciseIds, onAddExercise, onClose }: ExercisePickerSheetProps) {
  ...
  return createPortal(
    <div
      ...
      className={`workout-dark fixed inset-0 z-[70] flex items-end justify-center bg-black/70 px-0 transition-opacity duration-[250ms] ease-out motion-reduce:transition-none sm:px-4 ${entered ? 'opacity-100' : 'opacity-0'}`}
      onMouseDown={(event) => {
    ...
    >
      <div
        ...
        className={`flex h-[86dvh] w-full max-w-[680px] flex-col overflow-hidden rounded-t-2xl border border-b-0 border-white/15 bg-[#111410] text-white transition-transform duration-[250ms] ease-out motion-reduce:transition-none sm:h-[82dvh] ${entered ? 'translate-y-0' : 'translate-y-full'}`}
      >
        <div className="shrink-0 border-b border-white/10 px-4 pb-3 pt-2 sm:px-5">
          ...
          <label className="relative mt-3 block">
            ...
            <input
              ...
              className="min-h-12 w-full rounded-xl border border-white/10 bg-white/[0.045] pl-9 pr-3 text-sm text-white outline-none transition placeholder:text-zinc-500 focus:border-lime-300/60 focus:ring-2 focus:ring-lime-300/15"
            />
          </label>

          <nav aria-label="动作肌群分类" className="mt-3 flex snap-x gap-1.5 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto overflow-x-hidden px-4 pb-[calc(1rem+env(safe-area-inset-bottom))] pt-3 sm:px-5">
          {mode === 'list' ? (
            ...
            <div data-testid="exercise-picker-2d-mode" className="space-y-3">
              ...
              <div className="grid gap-3 min-[520px]:grid-cols-[minmax(190px,0.78fr)_minmax(0,1.22fr)]">
  ...
  return (
    <div className="space-y-2">
      {exercises.map((exercise) => {
        ...
        return (
          <article key={exercise.id} data-testid={`exercise-picker-result-${exercise.id}`} className="flex min-w-0 items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.025] p-3">
