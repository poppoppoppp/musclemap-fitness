      ...
      <div className="mt-6 space-y-5">
        {sessions.map((session) => {
          ...
          return <article key={session.id} className="rounded-xl border border-white/10 bg-white/[0.03] px-4 py-4">
            ...
            {session.captureSnapshot ? <p className="mt-3 rounded-lg border border-white/10 px-3 py-2 text-xs leading-5 text-zinc-300">自动采集：{captureValidityLabel(session.captureSnapshot.validity)} · 静态 {session.captureSnapshot.staticCaptures.length}/3 · 动态 {session.captureSnapshot.movements.length}/3</p> : null}
...
function captureValidityLabel(validity: NonNullable<PostureScreeningSession['captureSnapshot']>['validity']): string { return validity === 'valid' ? '有效' : validity === 'partial' ? '部分有效' : '无效'; }
